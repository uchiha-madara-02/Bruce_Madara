...
** Giữ nguyên logic gốc, chỉ thêm hiệu ứng trượt cho MENU_TYPE_SUBMENU
**********************************************************************/
int loopOptions(
    ...
    static unsigned long menuOpenTs = 0;
    drawMainBorder();
    ...
    while (1) {
        ...
        if (redraw) {
            ...
            if (!renderedByLambda) {
                // Nếu là submenu, gọi vẽ bình thường. Hiệu ứng trượt sẽ được xử lý riêng ở phần xử lý nút bấm
        ...
        if (PrevPress || check(UpPress)) {
            devModeCounter = 0;
            int oldIndex = index;      // Lưu lại index cũ cho hiệu ứng
            bool indexChanged = false; // Cờ kiểm tra xem có chuyển menu không

#ifdef HAS_KEYBOARD
            check(PrevPress);
            if (index == 0) index = options.size() - 1;
            else if (index > 0) index--;
            indexChanged = true; // Có thay đổi index
#else
            long _tmp = millis();
#ifndef HAS_ENCODER // T-Embed không cần phần này
            LongPress = true;
            ...
            // Xóa vòng cung sau khi nhả phím
            tft.drawArc(
                tftWidth / 2, tftHeight / 2, 25, 15, 0, 360, bruceConfig.bgColor, bruceConfig.bgColor
            );
            LongPress = false;
#endif              // Kết thúc #ifndef HAS_ENCODER

            if (millis() - _tmp > 700) { // Phát hiện nhấn giữ (Long press) để thoát
                index = -1;
                break; // Thoát menu, không chạy hiệu ứng trượt
            } else {   // Nhấn nhả (Short press) -> Chuyển menu
                check(PrevPress);
                if (index == 0) index = options.size() - 1;
                else if (index > 0) index--;
                indexChanged = true; // Có thay đổi index
            }
#endif              // Kết thúc #ifdef HAS_KEYBOARD

            // [HIỆU ỨNG TRƯỢT] Chỉ kích hoạt nếu thực sự có chuyển menu và là menu con
            if (indexChanged) {
                if (menuType == MENU_TYPE_SUBMENU) {
                    float curve[] = {0.45, 0.85}; // Offset trượt
                    for (int c = 0; c < 2; c++) {
                        drawSubmenu(oldIndex, options, subText, curve[c]);
                        vTaskDelay(10 / portTICK_PERIOD_MS);
                    }
                }
                redraw = true;
        ...
        if (check(NextPress) || check(DownPress)) {
            int oldIndex = index; // Lưu lại index cũ cho hiệu ứng trượt
            index++;
            ...
            // [HIỆU ỨNG TRƯỢT] Chỉ kích hoạt nếu là menu con
            if (menuType == MENU_TYPE_SUBMENU) {
                float curve[] = {-0.45, -0.85}; // Offset trượt ngược
                for (int c = 0; c < 2; c++) {
                    drawSubmenu(oldIndex, options, subText, curve[c]);
                    vTaskDelay(10 / portTICK_PERIOD_MS);
                }
        ...
        if (forceMenuOption >= 0 || (millis() - menuOpenTs > 600 && check(SelPress))) {
...
** Description:   Hàm vẽ Regular Menu có hiệu ứng cuộn mượt (Anti-Flicker)
***************************************************************************************/
Opt_Coord drawOptions(
    int index, std::vector<Option> &options, uint16_t fgcolor, uint16_t selcolor, uint16_t bgcolor,
    bool firstRender, float animOffset
) {
    Opt_Coord coord;
    int menuSize = options.size();
    if (menuSize > MAX_MENU_SIZE) menuSize = MAX_MENU_SIZE;

    int itemH = FM * 8 + 4; // Chiều cao của 1 dòng
    int optionsTopY = tftHeight / 2 - menuSize * itemH / 2 - 5;
    int boxX = tftWidth * 0.10;
    int boxW = tftWidth * 0.8;
    int boxH = itemH * menuSize + 10;

    // --- 1. TÍNH TOÁN TỌA ĐỘ TRƯỢT ---
    float currentIdxFloat = index + animOffset;
    float actualScroll = 0;

    // Nếu danh sách dài, tính toán độ đẩy trang
    if (options.size() > MAX_MENU_SIZE) {
        if (currentIdxFloat > MAX_MENU_SIZE - 1) { actualScroll = currentIdxFloat - MAX_MENU_SIZE + 1; }
        if (actualScroll > options.size() - MAX_MENU_SIZE) { actualScroll = options.size() - MAX_MENU_SIZE; }
        if (actualScroll < 0) actualScroll = 0;
    }

    // Độ lướt của con trỏ ">"
    float actualCursor = currentIdxFloat - actualScroll;

    // --- 2. THUẬT TOÁN XÓA NỀN THÔNG MINH (TRỊ BỆNH NHÁY) ---
    static float lastScroll = -1;
    // Kiểm tra xem trang chữ có đang bị cuộn vật lý hay không
    bool isTextMoving = false;
    if (!firstRender && abs(actualScroll - lastScroll) > 0.001) { isTextMoving = true; }

    if (firstRender) {
        tft.drawPixel(0, 0, bruceConfig.bgColor);
        tft.fillRoundRect(boxX, optionsTopY, boxW, boxH, 5, bgcolor);
        tft.drawRoundRect(boxX, optionsTopY, boxW, boxH, 5, fgcolor);
    } else {
        if (isTextMoving) {
            // Nếu cả trang chữ đang trượt -> Buộc phải xóa toàn bộ hộp
            tft.fillRect(boxX + 2, optionsTopY + 2, boxW - 4, boxH - 4, bgcolor);
        } else {
            // Nếu chữ ĐỨNG IM -> CHỈ xóa cái cột dọc tí xíu chứa con trỏ ">"
            tft.fillRect(boxX + 2, optionsTopY + 2, (FM * LW) + 4, boxH - 4, bgcolor);
        }
    }
    lastScroll = actualScroll;

    // --- 3. BẬT KHIÊN BẢO VỆ CHỐNG TRÀN ---
#if defined(USE_M5GFX)
    M5.Display.setClipRect(boxX + 2, optionsTopY + 2, boxW - 4, boxH - 4);
#else
    ((TFT_eSPI *)&tft)->setViewport(boxX + 2, optionsTopY + 2, boxW - 4, boxH - 4, false);
#endif

    tft.setTextSize(FM);
    int maxChars = (boxW - 10) / (LW * FM) - 1;

    // --- 4. VẼ CÁC DÒNG CHỮ ---
    for (int i = 0; i < options.size(); i++) {
        float yPos = optionsTopY + 5 + (i - actualScroll) * itemH;

        // Chỉ vẽ những chữ nằm trong tầm nhìn
        if (yPos > optionsTopY - itemH && yPos < optionsTopY + boxH) {

            // Đổi màu êm ái khi con trỏ lướt ngang qua
            uint16_t color = (abs(i - currentIdxFloat) < 0.5) ? selcolor : fgcolor;

            if (isTextMoving) {
                // ĐANG TRƯỢT: Dùng chữ trong suốt (giống Submenu) cho mượt
                tft.setTextColor(color);
                String text = options[i].label;
                tft.drawString(text.substring(0, maxChars).c_str(), boxX + 5 + FM * LW, (int)yPos);
            } else {
                // ĐỨNG IM: Dùng chữ đục (có nền) tự đè lên chữ cũ, đảm bảo tĩnh 100%
                tft.setTextColor(color, bgcolor);
                String text = String(options[i].label) + "                  ";
                tft.drawString(text.substring(0, maxChars).c_str(), boxX + 5 + FM * LW, (int)yPos);
            }

            if (i == index && animOffset == 0.0) {
    ...
    // --- 5. VẼ CON TRỎ LƯỚT CHUYÊN BIỆT ---
    float cursorY = optionsTopY + 5 + actualCursor * itemH;
    if (isTextMoving) tft.setTextColor(selcolor);
    else tft.setTextColor(selcolor, bgcolor);

    tft.drawString(">", boxX + 5, (int)cursorY);
...
** Description:   Hàm vẽ Submenu có hiệu ứng cuộn mượt (Smooth Scrolling Wheel)
***************************************************************************************/
void drawSubmenu(int index, std::vector<Option> &options, const char *title, float animOffset) {
    int menuSize = options.size();
    int middle = 25 /*status*/ + (tftHeight - 30 /*status + margin*/) / 2;
    int gap = (tftHeight - 42) / 3; // Khoảng cách giữa các dòng

    // --- 1. CHỈ VẼ HEADER & BAR KHI KHÔNG TRƯỢT (CHỐNG NHÁY) ---
    if (animOffset == 0.0) {
        drawStatusBar();
...
#if defined(HAS_TOUCH)
        tft.setTextSize(FM);
        tft.drawCentreString("/\\", tftWidth / 2, middle - gap - (FM * LH + 6), 1);
        tft.drawCentreString("\\/", tftWidth / 2, middle + gap + (FM * LH + 6), 1);
    ...
    // --- 2 & 3. KHỞI TẠO SPRITE (ẢNH NHÁP TRÊN RAM) ĐỂ CHỐNG NHÁY ---
    int viewY = 46;
    int viewH = tftHeight - viewY - 15;
    int spriteWidth = tftWidth - 14;

// Lưu ý: Nếu dùng thư viện M5GFX thuần, bạn có thể thay bằng LGFX_Sprite
#if defined(USE_M5GFX)
    M5Canvas sprite((LovyanGFX *)&tft);
#else
    TFT_eSprite sprite = TFT_eSprite((TFT_eSPI *)&tft);
#endif
    sprite.createSprite(spriteWidth, viewH);
    sprite.fillSprite(bruceConfig.bgColor); // Xóa nền trên RAM, màn hình KHÔNG bị nháy

    // Vẽ vòng quay 5 item lên Sprite
    for (int i = -2; i <= 2; i++) {
        int itemIdx = (index + i) % menuSize;
        if (itemIdx < 0) itemIdx += menuSize;

        float pos = i + animOffset;
        float dist = abs(pos);
        ...
        if (dist <= 0.6) {
            color = bruceConfig.priColor;
            if (options[itemIdx].label.length() <= tftWidth / (LW * FG) - 1) textSize = FG;
        }

        // Tọa độ Y ban đầu trên màn hình
        int y_screen = middle + (int)(pos * gap) - (textSize * LH) / 2;
        // Trừ đi viewY để quy đổi về tọa độ bên trong Sprite (Sprite bắt đầu từ y=0)
        int y_sprite = y_screen - viewY;

        sprite.setTextSize(textSize);
        sprite.setTextColor(color);
        // Tâm chữ là ở giữa chiều rộng của Sprite
        sprite.drawCentreString(options[itemIdx].label.c_str(), spriteWidth / 2, y_sprite, SMOOTH_FONT);

        // Kẻ gạch chân dưới cho mục chính giữa
        if (dist <= 0.6) {
            int lineW = strlen(options[itemIdx].label.c_str()) * textSize * LW;
            int startX = spriteWidth / 2 - lineW / 2;
            sprite.drawFastHLine(startX, y_sprite + textSize * LH + 1, lineW, bruceConfig.priColor);
        }
    }

    // Đẩy toàn bộ Sprite lên màn hình tại tọa độ (6, viewY)
    sprite.pushSprite(6, viewY);
    sprite.deleteSprite(); // Giải phóng RAM

    // --- 5. [FIX LỖI] XỬ LÝ TITLE KHÔNG BỊ NHÁY ---
    // Sử dụng màu nền text thay vì dùng fillRect để xóa (giảm đáng kể nháy)
    tft.setTextColor(bruceConfig.priColor, bruceConfig.bgColor);
    tft.setTextSize(FP);

    // Chỉ vẽ lại Title nếu không trong trạng thái trượt, hoặc đảm bảo background đã gánh phần xóa
    if (animOffset == 0.0) {
        // Chỉ dùng fillRect khi animOffset == 0 để tránh chớp giật khi đang cuộn
        tft.fillRect(6, 30, tftWidth - 12, 12, bruceConfig.bgColor);
