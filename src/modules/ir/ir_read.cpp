...
void IrRead::begin() {
    _read_signal = false;     // Đánh dấu chưa đọc được tín hiệu
    _banner_drawn_ir = false; // QUAN TRỌNG: Cho phép vẽ lại Graphic Banner cho lượt mới

    // Gọi hàm hiển thị logo đồ họa
    // (Hàm này đã bao gồm logic hiện tên phím cách đáy 10px mà bạn yêu cầu)
    display_graphic_banner();

    // Delay một chút để ổn định phần cứng IR sau khi khởi tạo giao diện
    delay(300);
}
...
void IrRead::display_graphic_banner() {
    if (_banner_drawn_ir) return; // Chốt chặn chống nháy
    ...
    if (maxDim >= 320) {
        ...
        M5.Display.drawBitmap(
            (tftWidth - 320) / 2, (tftHeight - 100) / 2, ir_read_320x100, 320, 100, bruceConfig.priColor
        );
#else
        ((TFT_eSPI *)&tft)
            ->drawBitmap(
...
#if defined(USE_M5GFX)
        M5.Display.drawBitmap(
            (tftWidth - 240) / 2, (tftHeight - 75) / 2, ir_read_240x75, 240, 75, bruceConfig.priColor
        );
#else
        ((TFT_eSPI *)&tft)
            ->drawBitmap(
    ...
    // 2. VẼ THÔNG TIN PHÍM BẤM TRONG KHUNG (CÁCH ĐÁY 10PX)
    if (quickloop && button_pos < quickButtons.size()) {
        String targetStr = "Target: " + String(quickButtons[button_pos]);

        // Thiết lập kích thước cho Font FM (Size 2)
        int charW = 12;    // Chiều rộng font FM
        int charH = 16;    // Chiều cao font FM
        int paddingX = 12; // Khoảng đệm ngang
        int paddingY = 6;  // Khoảng đệm dọc

        int textW = targetStr.length() * charW;
        int textH = charH;

        // Tính tọa độ căn giữa và cách đáy 10px
        int textX = (tftWidth - textW) / 2;
        int textY = tftHeight - 10 - textH;

        // Tọa độ và kích thước khung
        ...
        // --- VẼ KHUNG BO GÓC ---
        // Vẽ viền kép để tăng độ dày cho cân xứng với font FM
        tft.drawRoundRect(frameX, frameY, frameW, frameH, 6, bruceConfig.priColor);
        tft.drawRoundRect(frameX - 1, frameY - 1, frameW + 2, frameH + 2, 6, bruceConfig.priColor);
...
void IrRead::read_signal() {
    ...
    _read_signal = true;
    _banner_drawn_ir = false; // Reset để lượt chờ sau lại hiện được logo

    // THÊM DÒNG NÀY VÀO ĐỂ ÉP LƯU DỮ LIỆU RAW
    raw = true;

    // Vẽ giao diện kết quả (Khung viền + Thông tin RAW)
    display_banner();
