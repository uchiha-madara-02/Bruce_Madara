...
void EMVReader::display_graphic_banner() {
    if (_banner_drawn_emv) return; // Nếu đã vẽ rồi thì không vẽ lại
...
std::vector<uint8_t> EMVReader::emv_ask_for_aid() {
    ...
    _banner_drawn_emv = false; // Reset trạng thái vẽ khi bắt đầu đợi thẻ mới
    ...
    while (!_cancelled) {
        ...
        // HIỂN THỊ LOGO TRONG KHI CHỜ
        display_graphic_banner();

        // Thử đọc thẻ
        if (nfc->readPassiveTargetID(PN532_MIFARE_ISO14443A, uid, &len, 100)) {
            // Khi tìm thấy thẻ, đánh dấu để các hàm hiển thị sau có thể vẽ chữ
            _banner_drawn_emv = false;
            ...
            if (nfc->EMVinDataExchange(ask_for_aid_apdu, sizeof(ask_for_aid_apdu), response, &response_len)) {
                ...
                if (Tlv.GetValue("4F", &aid) != OK) {
                    Serial.println("Can't get aid");
                    aid.clear();
                }
                Serial.println("Success AID");
            }
            break; // Thoát vòng lặp khi đã tương tác xong với thẻ
        }
        delay(10); // Delay nhỏ để tránh chiếm dụng CPU quá mức
    }
    return aid;
}
...
void EMVReader::display_emv(EMVCard card) {
    ...
    // GIỮ NGUYÊN CÁCH NHẤN CŨ
    while (!AnyKeyPress) { delay(100); }

    // Sau khi nhấn phím, chúng ta chuẩn bị cho lượt quét mới
    _banner_drawn_emv = false;
