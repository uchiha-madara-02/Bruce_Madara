...
void TagOMatic::display_graphic_banner() {
    if (_banner_drawn_rfid) return;

    tft.fillScreen(bruceConfig.bgColor);
    tft.drawRoundRect(0, 0, tftWidth, tftHeight, 20, bruceConfig.priColor);
    int maxDim = max(tftWidth, tftHeight);

    // KIỂM TRA ĐIỀU KIỆN DỰA TRÊN CHIỀU DÀI MÀN HÌNH
    if (maxDim >= 320) {
...
void TagOMatic::set_state(RFID_State state) {
    current_state = state;
    _banner_drawn_rfid = false;

    if (state != READ_MODE && state != SCAN_MODE) { display_banner(); }
...
void TagOMatic::read_card() {
    if (millis() - _lastReadTime < 500) return;

    bool hasCard = false;
    if (_rfid->read() == RFIDInterface::SUCCESS) {
        hasCard = true;
    } else if (bruceConfigPins.rfidModule != M5_RFID2_MODULE) {
        if (_rfid->read(1) == RFIDInterface::SUCCESS) { hasCard = true; }
    }
    ...
    // VÒNG LẶP CHỜ ĐA NĂNG
    while (true) {
        if (check(NextPress) || check(PrevPress)) {
            break; // Thoát ra để quay lại màn hình chờ quét thẻ tiếp theo
        }

        if (check(SelPress)) {
            select_state(); // Mở menu tùy chọn (Clone, Save...)
            return;         // Sau khi chọn xong, thoát hàm để loop() chính xử lý trạng thái mới
        }
...
void TagOMatic::scan_cards() {
    ...
    while (true) {
        ...
        if (check(SelPress)) {
            select_state(); // Cho phép đổi chế độ hoặc lưu kết quả quét
            return;
        }
