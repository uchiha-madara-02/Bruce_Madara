...
void SRIXTool::display_graphic_banner() {
    if (_screen_drawn) return; // Chống nháy: đã vẽ rồi thì không vẽ lại
...
bool SRIXTool::waitForTag() {
    if (millis() - _lastReadTime < 1000) return false;

    // Hiển thị hình ảnh logo thay vì dòng chữ "Waiting..."
    display_graphic_banner();

    // Thử kích hoạt thẻ SRIX
    if (nfc->SRIX_initiate_select()) {
        _screen_drawn = false; // Reset biến để các hàm sau có thể vẽ dữ liệu thẻ
        return true;
    }
...
void SRIXTool::read_tag() {
    ...
    while (true) {
        if (check(NextPress) || check(PrevPress)) {
            _screen_drawn = false; // Reset để quay về vẽ lại Logo chờ
            break;
        }
        if (check(SelPress)) {
            select_state(); // Mở menu tùy chọn
            return;
        }
        if (check(EscPress)) {
            returnToMenu = true;
