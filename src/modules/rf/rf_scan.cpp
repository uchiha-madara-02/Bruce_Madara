...
bool _banner_drawn_rf = false;
static String lastFreqStr = "";

void RFScan::setup() {
    _banner_drawn_rf = false; // Ép vẽ lại banner mới

    if (!initRfModule("rx", bruceConfigPins.rfFreq)) { return; }
    RCSwitch_Enable_Receive(rcswitch);

    // Bắt buộc Reset frequency về 0 nếu không dùng tần số cố định
    if (bruceConfigPins.rfFxdFreq) {
        frequency = bruceConfigPins.rfFreq;
    } else {
        frequency = 0; // Để nó hiện Range thay vì hiện số MHz cũ
    }

    // --- SỬA TẠI ĐÂY ---
    // XÓA dòng cũ: display_info(received, signals, ReadRAW, codesOnly, autoSave, title);
    // THÊM dòng mới: Gọi giao diện Đồ họa (Logo + Khung) khi bắt đầu quét
    display_graphic_banner();
    // -------------------
...
void RFScan::display_graphic_banner() {
    // 1. Xác định nội dung chữ (Giữ nguyên logic cũ)
    String freqStr = "";
    if (frequency > 0 && !bruceConfigPins.rfFxdFreq) {
        freqStr = String(frequency, 2) + " MHz";
    } else if (bruceConfigPins.rfFxdFreq) {
        freqStr = "Fixed: " + String(bruceConfigPins.rfFreq, 2) + " MHz";
    } else {
        freqStr = "Scan: " + String(subghz_frequency_ranges[bruceConfigPins.rfScanRange]);
    }

    // 2. Vẽ Logo lần đầu (Giữ nguyên logic cũ)
    if (!_banner_drawn_rf) {
        tft.fillScreen(bruceConfig.bgColor);
        tft.drawRoundRect(0, 0, tftWidth, tftHeight, 20, bruceConfig.priColor);
        int iconW = (tftWidth >= 320) ? 320 : 240;
        int iconH = (tftWidth >= 320) ? 136 : 102;
        const unsigned char *bitmap = (tftWidth >= 320) ? rf_320x136 : rf_240x102;
#if defined(USE_M5GFX)
        M5.Display.drawBitmap(
            (tftWidth - iconW) / 2, (tftHeight - iconH) / 2 - 5, bitmap, iconW, iconH, bruceConfig.priColor
        );
#else
        ((TFT_eSPI *)&tft)
            ->drawBitmap(
        ...
        _banner_drawn_rf = true;
        lastFreqStr = ""; // Ép vẽ lại lần đầu
    }

    // 3. CHỐNG NHÁY & VẼ KHUNG: Chỉ thực hiện khi nội dung thay đổi
    if (freqStr != lastFreqStr) {
        // --- BẮT BUỘC: Đặt font FM trước khi đo kích thước ---
        tft.setTextSize(FM);

        // --- TÍNH TOÁN KÍCH THƯỚC KHUNG (Tự động đo) ---
        int paddingX = 8; // Đệm ngang (cho font to thì đệm rộng ra một chút cho đẹp)
        int paddingY = 6; // Đệm dọc

        // Tự động lấy chính xác chiều rộng và chiều cao của chuỗi theo font FM
        int textW = tft.textWidth(freqStr);
        int textH = tft.fontHeight();

        // Tọa độ chữ (Căn giữa, cách đáy 15px)
        int textX = (tftWidth - textW) / 2;
        int textY = tftHeight - 15 - textH;

        // Tọa độ và kích thước khung bao quanh
        int frameX = textX - paddingX;
        int frameY = textY - paddingY;
        int frameW = textW + (paddingX * 2);
        int frameH = textH + (paddingY * 2);

        // --- BẮT ĐẦU VẼ ---

        // a. Xóa sạch vùng cũ (Do font FM to hơn, mình tăng chiều cao vùng xóa lên 45px)
        tft.fillRect(10, tftHeight - 50, tftWidth - 20, 45, bruceConfig.bgColor);

        // b. Vẽ khung bo góc (Rounded Rectangle)
        tft.drawRoundRect(frameX, frameY, frameW, frameH, 4, bruceConfig.priColor);

        // c. Vẽ chữ lên trên khung
        tft.setTextColor(bruceConfig.priColor);
        tft.setCursor(textX, textY);
        tft.print(freqStr);

        lastFreqStr = freqStr; // Cập nhật trạng thái
    }
}
...
bool RFScan::fast_scan() {
    // Đảm bảo không bị tràn mảng dải tần
    if (idx < range_limits[bruceConfigPins.rfScanRange][0] ||
        idx > range_limits[bruceConfigPins.rfScanRange][1]) {
        idx = range_limits[bruceConfigPins.rfScanRange][0];
    }
    ...
    // FIX TỐC ĐỘ: Thay vì delay 5ms, ta dùng delayMicroseconds siêu nhỏ
    // chỉ đủ để thanh ghi CC1101 ổn định (Phase Lock Loop)
    delayMicroseconds(800);

    rssi = ELECHOUSE_cc1101.getRssi();

    // Nếu phát hiện sóng vượt ngưỡng
    if (rssi > rssiThreshold) {
        _freqs[_try].freq = checkFrequency;
        _freqs[_try].rssi = rssi;
        _try++;

        // Đã thu đủ mẫu để chốt tần số
        if (_try >= _MAX_TRIES) {
            int max_index = 0;
            // Tìm mẫu có tín hiệu mạnh nhất (RSSI cao nhất)
            for (int i = 1; i < _MAX_TRIES; ++i) {
                if (_freqs[i].rssi > _freqs[max_index].rssi) { max_index = i; }
            }

            bruceConfigPins.setRfFreq(_freqs[max_index].freq, 2); // Chốt tần số
            frequency = _freqs[max_index].freq;
            setMHZ(frequency);

            Serial.println(
                "Frequency Found: " + String(frequency) + " MHz (RSSI: " + String(_freqs[max_index].rssi) +
                ")"
            );

            // Vẽ UI báo hiệu chớp nhoáng
            tft.fillRect(10, 20, tftWidth - 20, 20, bruceConfig.bgColor);
            tft.setCursor(10, 25);
            tft.setTextColor(TFT_GREEN, bruceConfig.bgColor);
            tft.setTextSize(FP);
            tft.print("LOCKED: " + String(frequency) + " MHz");
    ...
    // Nhả CPU mỗi 10 vòng lặp để ESP32 không bị treo Watchdog (quan trọng khi quét siêu tốc)
    if (idx % 10 == 0) vTaskDelay(1 / portTICK_PERIOD_MS);
...
// }
void RFScan::read_raw() {
    // CHỜ TÍN HIỆU ĐẦY BỘ ĐỆM: Thay vì đợi cứng 400ms, ta chia nhỏ để thoát nhanh nếu đã nhận xong
    for (int w = 0; w < 10; w++) {
        vTaskDelay(20 / portTICK_PERIOD_MS);
        if (!rcswitch.RAWavailable()) break; // Nếu thư viện đã ngừng thu -> ngắt sớm
    }
    ...
    // --- NÂNG CẤP: AUTO-TRIMMING (CẮT TỈA RÁC) ---
    // 1. Tìm điểm bắt đầu hợp lệ (Bỏ qua rác nhiễu < 100us ở đầu)
    int start_idx = 1; // Bỏ qua phần tử 0 (thường là thời gian chờ rác)
    while (start_idx < RCSWITCH_RAW_MAX_CHANGES && raw[start_idx] < 100 && raw[start_idx] != 0) {
        start_idx++;
    }

    // 2. Tìm điểm kết thúc hợp lệ
    int end_idx = start_idx;
    while (end_idx < RCSWITCH_RAW_MAX_CHANGES && raw[end_idx] != 0) { end_idx++; }
    // Lùi lại để cắt bỏ cái đuôi chờ rác khổng lồ (thường > 20000us)
    if (end_idx > start_idx && raw[end_idx - 1] > 20000) { end_idx--; }

    String _data = "";
    received.te = 0;

    // 3. Xây dựng lại chuỗi RAW hoàn hảo
    for (transitions = start_idx; transitions < end_idx; transitions++) {
        if (transitions > start_idx) _data += " ";

        // Xác định cực: Xung lẻ là HIGH (+), xung chẵn là LOW (-) (Tính từ start_idx)
        signed int sign = ((transitions - start_idx) % 2 == 0) ? 1 : -1;
        int duration = sign * (int)raw[transitions];
    ...
    // Lọc nhiễu tĩnh: Nếu số lượng xung quá ít (chỉ là cái xẹt điện), hủy luôn!
    int actual_transitions = end_idx - start_idx;
    if (actual_transitions < 20 && !decoded) {
        rcswitch.resetAvailable();
        return; // Bỏ qua, tiếp tục lắng nghe
    }
    ...
    // Nếu vừa có mã Decoded (chuẩn RCSwitch) vừa có RAW
    if (decoded) {
        Serial.println("RcSwitch signal captured");
        blinkLed();
        ++signals;
    ...
    // Nếu chỉ là RAW (Nhưng có tính được CRC)
    else if (repetition >= 2 && !durations.empty()) {
        Serial.println("Raw signal with CRC captured");
        blinkLed();
        ++signals;
        received.preset = "FuriHalSubGhzPresetOok650Async"; // Preset băm OOK chuẩn
        received.protocol = "RAW";
        received.key = crc64_ecma(durations);
        received.indexed_durations = indexed_durations;
        received.Bit = actual_transitions; // Lưu chính xác số lượng xung
        frequency = 0;
        display_info(received, signals, ReadRAW, codesOnly, autoSave, title);
    }
    // Nếu hoàn toàn là RAW Data vô danh
    else if (!codesOnly) {
        Serial.println("Raw static data captured");
        blinkLed();
        ++signals;
        ...
        received.key = 0;
        received.indexed_durations = {};
        received.Bit = actual_transitions;
        frequency = 0;
        display_info(received, signals, ReadRAW, codesOnly, autoSave, title);
    }
